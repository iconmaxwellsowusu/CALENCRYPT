...
<% 
    if(request.getParameter("lincense")!=null)
    {
        ...
        pst = con.prepareStatement("update lincense set Dname=?,lincense=?,Date1=?,Date2=?,Date3=?,Date4=? where id = ?");
...
<%    
    response.sendRedirect("lincense.jsp");
    }
   %>
			...
			<div class="col-sm-9 col-xs-12 content pt-3 pl-0">
				...
				<form action="#" method="post">
					<%    
                        ...
                        ps = co.prepareStatement("select * from lincense where id = ?");
                        ps.setString(1, idd);
                        r = ps.executeQuery();
					...
					<div class="form-row">
						...
						<div class="col-md-4 mb-3">
							<label for="validationCustom03">Driver Name</label> <input
								...
								name="driver_name" value="<%=r.getString(2)%>">
						</div>



						<div class="col-md-4 mb-3">
							<label for="validationCustom03">Lincense Grade</label> <input
								type="text" class="form-control" placeholder="Name"
								name="lincense" value="<%=r.getString(3)%>">
						</div>
						...
						<div class="col-md-4 mb-3 input-with-post-icon datepicker">
							<label for="example">First Renewal Date</label> <input
								...
								class="form-control" name="date1" value="<%=r.getString(4)%>">
						...
						<div class="col-md-4 mb-3 input-with-post-icon datepicker">
							<label for="example">Second Renewal Date</label> <input
								placeholder="Select date" type="date" id="example"
								class="form-control" name="date2" value="<%=r.getString(5)%>">
						...
						<div class="col-md-4 mb-3 input-with-post-icon datepicker">
							<label for="example">Third Renewal Date</label> <input
								placeholder="Select date" type="date" id="example"
								class="form-control" name="date3" value="<%=r.getString(6)%>">
						...
						<div class="col-md-4 mb-3 input-with-post-icon datepicker">
							<label for="example">Expiration Date</label> <input
								placeholder="Select date" type="date" id="example"
								class="form-control" name="date4" value="<%=r.getString(7)%>">
					...
					<%} %>
					<button type="submit" id="lincense" name="lincense" value="submit"
							...
							<table class="table table-bordered table-hover table-fixed"
								...
								<thead class="thead-light">
									...
									<tr>
										...
										<th scope="col">Lincense Type</th>

										<th scope="col">First Renewal Date</th>
										<th scope="col">Second Renewal Date</th>
										<th scope="col">Third Renewal Date</th>
                                ...
                                con = DriverManager.getConnection("jdbc:mysql://localhost/kodsonvms?useUnicode=true&useJDBCCompliantTimezoneShift=true&useLegacyDatetimeCode=false&serverTimezone=UTC","kodson_maxwell","0040105715@Icon");
                                  ...
                                  String query = "SELECT id, Dname,lincense,Date1,Date2,Date3,Date4, FLOOR(TIMESTAMPDIFF(MINUTE, NOW(), Date4) / 1440) as Days, FLOOR(TIMESTAMPDIFF(MINUTE, NOW(), Date4) % 1440 / 60) as Hours, TIMESTAMPDIFF(MINUTE, NOW(), Date4) % 60 as Minutes FROM lincense   where FLOOR(TIMESTAMPDIFF(MINUTE, NOW(), Date4) / 1440) >=1";
                                 ...
                                 if(dayys>=30){
                                 out.println("<span class=\"badge badge-success\">"+rs.getString(8)+" (days) "+rs.getString(9)+" (hours) "+rs.getString(10)+" (munites)"+"</span>");
                                 }else{
                                	 out.println("<span class=\"badge badge-danger\">"+rs.getString(8)+" (days) "+rs.getString(9)+" (hours) "+rs.getString(10)+" (munites)"+"</span>");
                                	 %> <%
                                	 
                                 }
                                 %>
										</td>
										<td><a href="updateLincense.jsp?id=<%=id%>"><i
												class="fa fa-pencil text-warning"> </i></a></td>
										<td><a href="deleteLicense.jsp?id=<%=id%>"
