...
<% 
    if(request.getParameter("factory")!=null)
    {
        ...
        pst = con.prepareStatement("update factory set registrationDate=?,expiryDate=? where id=?");
...
<% 
    response.sendRedirect("factoryInspection.jsp");
    }
   %>
	...
	<div class="container-fluid">
		...
		<div class="row main-content">
			...
			<div class="col-sm-9 col-xs-12 content pt-3 pl-0">
				<h5 class="mb-3">
					<strong>update Factory Inspection</strong>
				</h5>
				...
				<form action="#" method="post">
					...
					<%    
                        ...
                        ps = co.prepareStatement("select * from factory where id = ?");
                        ps.setString(1, idd);
                        r = ps.executeQuery();
						...
						<div class="scrollable">
							<table class="table table-bordered table-hover table-fixed"
                                ...
                                con = DriverManager.getConnection("jdbc:mysql://localhost/kodsonvms?useUnicode=true&useJDBCCompliantTimezoneShift=true&useLegacyDatetimeCode=false&serverTimezone=UTC","kodson_maxwell","0040105715@Icon");
                                  ...
                                  String query = "SELECT id, registrationDate,expiryDate, FLOOR(TIMESTAMPDIFF(MINUTE, NOW(), expiryDate) / 1440) as Days, FLOOR(TIMESTAMPDIFF(MINUTE, NOW(), expiryDate) % 1440 / 60) as Hours, TIMESTAMPDIFF(MINUTE, NOW(), expiryDate) % 60 as Minutes FROM factory   where FLOOR(TIMESTAMPDIFF(MINUTE, NOW(), expiryDate) / 1440) >=1";
                                 ...
                                 if(dayys>=30){
                                 out.println("<span class=\"badge badge-success\">"+rs.getString(4)+" (days) "+rs.getString(5)+" (hours) "+rs.getString(6)+" (munites)"+"</span>");
                                 }else{
                                	 out.println("<span class=\"badge badge-danger\">"+rs.getString(4)+" (days) "+rs.getString(5)+" (hours) "+rs.getString(6)+" (munites)"+"</span>");
                                	 %> <%
                                	 
                                 }
                                 %>
										</td>
										<td><a href="updateFactory.jsp?id=<%=id%>"><i
												class="fa fa-pencil text-warning"> </i></a></td>
										<td><a href="deleteFactory.jsp?id=<%=id%>"
