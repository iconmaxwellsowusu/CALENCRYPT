...
<% 
    if(request.getParameter("submit")!=null)
    {
        ...
        pst = con.prepareStatement("update goods_insurance set driver_name=?,truck_number=?,goods_insurance_number=?,registration_date=?,expiration_date=? where id = ?");
...
<% 
    response.sendRedirect("goodsinsurance.jsp");
    }
	...
	<div class="container-fluid">
				...
				<div class="bg-theme mr-3 pt-3 pb-2 mb-0">
					<h3 class="logo">
						<a href="#" class="text-secondary logo"><i class="fa fa-book"></i>
							Goods Insurance <span class="small">Page</span></a>
					</h3>
				</div>
			</div>
			<!--Logo-->
		...
		<div class="row main-content">
			...
			<div class="col-sm-3 col-xs-6 sidebar pl-0">
				<div class="inner-sidebar mr-3">
					...
					<div class="avatar text-center">
						...
						<span class="text-primary small"><strong>Goods
								Insurance Page</strong></span>
					</div>
					<!--Image Avatar-->
			...
			<div class="col-sm-9 col-xs-12 content pt-3 pl-0">
				<h5 class="mb-3">
					<strong>Update Goods Insurance</strong>
				</h5>
				...
				<form method="POST" action="#">
					...
					<%    
                        ...
                        ps = co.prepareStatement("select * from goods_insurance where id = ?");
                        ps.setString(1, i);
                        r = ps.executeQuery();
					...
					<div class="form-row">
						...
						<div class="col-md-4 mb-3">
							<label for="validationCustom03">Driver Name</label> <input
								...
								value="<%= r.getString("driver_name")%>" name="driver_name"
								required>
						</div>
						...
						<div class="col-md-4 mb-3">
							<label for="validationCustom05">Truck Number</label> <input
								type="text" class="form-control" placeholder=""
								value="<%= r.getString("truck_number")%>" name="truck_number"
								required>
						</div>

						<div class="col-md-4 mb-3">
							<label for="validationCustom05">Goods Insurance Number</label> <input
								type="text" class="form-control" placeholder=""
								value="<%= r.getString("goods_insurance_number")%>"
						...
						<div class="col-md-4 mb-3 input-with-post-icon datepicker">
							<label for="example">Registration Date</label> <input
								...
								value="<%= r.getString("registration_date")%>" type="date"
								id="example" class="form-control" name="registration_date">
						</div>
						...
						<div class="col-md-4 mb-3 input-with-post-icon datepicker">
							<label for="example">Expiration Date</label> <input
								placeholder="Select date"
								value="<%= r.getString("expiration_date")%>" type="date"
								id="example" class="form-control" name="expiration_date">
						</div>
					</div>
							...
							<table class="table table-bordered table-hover table-fixed"
								...
								<thead class="thead-light">
									...
									<tr>
										...
										<th scope="col">Goods Insurance Number</th>
                                ...
                                con = DriverManager.getConnection("jdbc:mysql://localhost/kodsonvms?useUnicode=true&useJDBCCompliantTimezoneShift=true&useLegacyDatetimeCode=false&serverTimezone=UTC","kodson_maxwell","0040105715@Icon");
                                  ...
                                  String query = "SELECT id, driver_name,truck_number,goods_insurance_number,registration_date,expiration_date, FLOOR(TIMESTAMPDIFF(MINUTE, NOW(), expiration_date) / 1440) as Days, FLOOR(TIMESTAMPDIFF(MINUTE, NOW(), expiration_date) % 1440 / 60) as Hours, TIMESTAMPDIFF(MINUTE, NOW(), expiration_date) % 60 as Minutes FROM goods_insurance   where FLOOR(TIMESTAMPDIFF(MINUTE, NOW(), expiration_date) / 1440) >=1";
                                  Statement st = con.createStatement();
                                   ...
                                   %>
									...
									<tr>
										...
										<td><%=rs.getString("truck_number") %></td>
										<td><%=rs.getString("goods_insurance_number") %></td>
										<td><%=rs.getString("registration_date") %></td>
										<td><%=rs.getString("expiration_date") %></td>
										<td>
											<%
                                 String d = rs.getString(7);
                                 int dayys = Integer.parseInt(d);
                                 if(dayys>=10){
                                 out.println("<span class=\"badge badge-success\">"+rs.getString(7)+" (days) "+rs.getString(8)+" (hours) "+rs.getString(9)+" (munites)"+"</span>");
                                 }else{
                                	 out.println("<span class=\"badge badge-danger\">"+rs.getString(7)+" (days) "+rs.getString(8)+" (hours) "+rs.getString(9)+" (munites)"+"</span>");
                                	 %> <script>alert("GOODS INSURANCE IS ABOUT TO EXPIRE") </script>
											<%
                                	 
                                 }
                                 %>
										</td>
										<td><a href="updateGoods.jsp?id=<%=id%>"><i
												class="fa fa-pencil text-warning"> </i></a></td>
										<td><a href="deleteGoods.jsp?id=<%=id%>"
