...
<% 
    if(request.getParameter("fire")!=null)
    {
        ...
        pst = con.prepareStatement("update fire set Dname=?,position=?,expiryDate=? where id=?");
...
<%   
    response.sendRedirect("fireTraining.jsp");
    }
   %>
			...
			<div class="col-sm-9 col-xs-12 content pt-3 pl-0">
				...
				<form action="#" method="post">
					<%    
                        ...
                        ps = co.prepareStatement("select * from fire where id = ?");
                        ps.setString(1, idd);
                        r = ps.executeQuery();
					...
					<div class="form-row">
						...
						<div class="col-md-4 mb-3">
							<label for="validationCustom03">Name</label> <input type="text"
						...
						<div class="col-md-4 mb-3">
							<label for="validationCustom05">position</label> <input
								type="text" class="form-control" placeholder="" name="position"
								value="<%=r.getString(3)%>">
						</div>
						...
						<div class="col-md-4 mb-3 input-with-post-icon datepicker">
							<label for="example"> Date</label> <input
								...
								class="form-control" name="date" value="<%=r.getString(4)%>">
						...
						<div class="scrollable">
							<table class="table table-bordered table-hover table-fixed"
                                ...
                                con = DriverManager.getConnection("jdbc:mysql://localhost/kodsonvms?useUnicode=true&useJDBCCompliantTimezoneShift=true&useLegacyDatetimeCode=false&serverTimezone=UTC","kodson_maxwell","0040105715@Icon");
                                  ...
                                  String query = "SELECT id, Dname,position,expiryDate, FLOOR(TIMESTAMPDIFF(MINUTE, NOW(), expiryDate) / 1440) as Days, FLOOR(TIMESTAMPDIFF(MINUTE, NOW(), expiryDate) % 1440 / 60) as Hours, TIMESTAMPDIFF(MINUTE, NOW(), expiryDate) % 60 as Minutes FROM fire   where FLOOR(TIMESTAMPDIFF(MINUTE, NOW(), expiryDate) / 1440) >=1";
                                 ...
                                 out.println("<span class=\"badge badge-success\">"+rs.getString(5)+" (days) "+rs.getString(6)+" (hours) "+rs.getString(7)+" (munites)"+"</span>");
                                	 ...
                                	 %> <script>alert("FIRE TRAINING ALERT") </script>
											<%
                                	 
                                 }
                                 %>
										</td>
										<td><a href="updateFire.jsp?id=<%=id%>"><i
												class="fa fa-pencil text-warning"> </i></a></td>
										<td><a href="deleteFire.jsp?id=<%=id%>"
