...
<% 
    if(request.getParameter("npaupdate")!=null)
    {
        ...
        pst = con.prepareStatement("update npa set Dname=?,Tnumber=?,capacity=?,registrationDate=?,expiryDate=?,amount=? where id = ?");
			...
			<div class="col-sm-9 col-xs-12 content pt-3 pl-0">
				...
				<form action="#" method="post">
					...
					<%    
                        ...
                        pst = con.prepareStatement("select * from npa where id = ?");
                        pst.setString(1, idd);
                        rs = pst.executeQuery();
					...
					<div class="form-row">
						...
						<div class="col-md-4 mb-3">
							<label for="validationCustom03">Driver Name</label> <input
								...
								name="driver_name" value="<%=rs.getString(2) %>">
						</div>
						...
						<div class="col-md-4 mb-3">
							<label for="validationCustom05">Amount</label> <input
								type="number" class="form-control" placeholder="" name="amount"
								value="<%=rs.getString(7) %>">
						</div>

					</div>
					<% }  %>
					<button type="submit" id="npa" name="npaupdate" value="submit"
						...
						<div class="scrollable">
							<table class="table table-bordered table-hover table-fixed"
                                ...
                                conn = DriverManager.getConnection("jdbc:mysql://localhost/kodsonvms?useUnicode=true&useJDBCCompliantTimezoneShift=true&useLegacyDatetimeCode=false&serverTimezone=UTC","kodson_maxwell","0040105715@Icon");
                                  ...
                                  String queryy = "SELECT id, Dname, Tnumber,capacity, registrationDate,expiryDate,amount, FLOOR(TIMESTAMPDIFF(MINUTE, NOW(), expiryDate) / 1440) as Days, FLOOR(TIMESTAMPDIFF(MINUTE, NOW(), expiryDate) % 1440 / 60) as Hours, TIMESTAMPDIFF(MINUTE, NOW(), expiryDate) % 60 as Minutes FROM npa   where FLOOR(TIMESTAMPDIFF(MINUTE, NOW(), expiryDate) / 1440) >1 ";
                                  Statement stt = conn.createStatement();
                                   ...
                                   %>
									...
									<tr>
										...
										<td><%=rss.getString(7) %> GHC</td>
										<td>
											<%
                                 String d = rss.getString(8);
                                 int dayys = Integer.parseInt(d);
                                 if(dayys>=31){
                                 out.println("<span class=\"badge badge-success\">"+rss.getString(8)+" (days) "+rss.getString(9)+" (hours) "+rss.getString(10)+" (munites)"+"</span>");
                                 }else{
                                	 out.println("<span class=\"badge badge-danger\">"+rss.getString(8)+" (days) "+rss.getString(9)+" (hours) "+rss.getString(10)+" (munites)"+"</span>");
                                 ...
                                 %>
										...
										<td><a href="updateNpa.jsp?id=<%=id%>"><i
												class="fa fa-pencil text-warning"> </i></a></td>
										<td><a href="deleteNpa.jsp?id=<%=id%>"
